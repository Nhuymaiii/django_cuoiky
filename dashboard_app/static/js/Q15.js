function drawChartQ15() {
    const svgId = "#chart-Q15";
    const svg = d3.select(svgId);
    svg.selectAll("*").remove();

    // Sử dụng q15_values từ Django view, giả định cấu trúc: [{policy: "flexible", average: 8.5}, ...]
    const averages = q15_values
        .map(item => ({
    ...
    const margin = { top: 30, right: 100, bottom: 30, left: 80 },
          ...
          height = averages.length * 40; // Điều chỉnh chiều cao cho tất cả chính sách hủy phòng
    ...
    const yScale = d3.scaleBand()
        .domain(averages.map(d => d.policy))
        .range([0, height])
        .padding(0.2);
    ...
    svgGroup.selectAll(".bar")
        ...
        .attr("fill", d => colorScale(d.policy))
        .on("mouseover", (event, d) => {
            tooltip.style("visibility", "visible")
                   .html(`<strong>Chính sách hủy phòng:</strong> ${d.policy}<br>
    ...
    svgGroup.selectAll(".label")
        ...
        .attr("y", d => yScale(d.policy) + yScale.bandwidth() / 2)
        .attr("dy", ".35em")
        .style("font-size", "10px")
    ...
    svgGroup.append("text")
        ...
        .style("font-family", "Calibri, sans-serif")
        .style("fill", "#d1495b")
        .text("Trung bình điểm theo chính sách hủy phòng");
}

// Xử lý lỗi
window.addEventListener('error', (event) => {
    if (event.message.includes("Cannot read properties of undefined")) {
        d3.select("#chart-Q15").append("text")
            .attr("x", 50)
            .attr("y", 50)
