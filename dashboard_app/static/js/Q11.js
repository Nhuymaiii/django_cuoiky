function drawChartQ11() {
    const svgId = "#chart-Q11";
    const svg = d3.select(svgId);
    svg.selectAll("*").remove();

    // Dữ liệu đã được xử lý trong view Django
    const counts = q11_values.map(item => ({
        status: item.status,
        count: item.count
    })).sort((a, b) => d3.descending(a.status, b.status)); // Sắp xếp theo thứ tự "Rồi" trước, "Chưa" sau

    // Giảm kích thước và lề
    const margin = { top: 30, right: 100, bottom: 30, left: 80 },
          width = 350 - margin.left - margin.right,
          height = counts.length * 100; // Giảm khoảng cách giữa các thanh
    ...
    svgGroup.selectAll(".bar")
        ...
        .on("mouseover", (event, d) => {
            tooltip.style("visibility", "visible")
                   .html(`<strong>Trạng thái Super Host:</strong> ${d.status}<br>
    ...
    svgGroup.append("text")
        ...
        .style("font-family", "Calibri, sans-serif")
        .style("fill", "#d1495b")
        .text("Số lượng nơi lưu trú theo Super Host");
}

// Xử lý lỗi (nếu có)
window.addEventListener('error', (event) => {
    if (event.message.includes("Cannot read properties of undefined")) {
        d3.select("#chart-Q11").append("text")
            .attr("x", 50)
            .attr("y", 50)
