function drawChartQ7() {
    ...
    const xScale = d3.scaleLinear()
        .domain([0, 100]) // Tỷ lệ phần trăm từ 0 đến 100
        .range([0, width]);

    const yScale = d3.scaleBand()
        .domain(ratios.map(d => d.name))
        .range([0, height])
        .padding(0.2);
        ...
        .on("mouseover", (event, d) => {
            tooltip.style("visibility", "visible")
                   .html(`<strong>Tiện ích:</strong> ${d.name}<br>
                          <strong>Tỷ lệ:</strong> ${d.ratio}%<br>
    ...
    // Nhãn số (hiển thị tỷ lệ phần trăm)
    svgGroup.selectAll(".label")
        .data(ratios)
        .enter().append("text")
        .attr("class", "label")
        .attr("x", d => xScale(d.ratio) + 2)
    ...
    svgGroup.append("text")
        ...
        .style("fill", "#d1495b")
        .text("Tỷ lệ nơi lưu trú có tiện ích an toàn");
}

// Xử lý lỗi (nếu có)
window.addEventListener('error', (event) => {
    if (event.message.includes("Cannot read properties of undefined")) {
        d3.select("#chart-Q7").append("text")
            .attr("x", 50)
            .attr("y", 50)
